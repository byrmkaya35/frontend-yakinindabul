...
function SearchMap() {
  ...
  function renderFilters() {
    if (isMobile) {
      // Mobilde yatay scroll ve trendyol scrollbar yok!
      return (
        <div
          className="mobile-filter-row hide-scrollbar w-full gap-2 pt-1 pb-2"
        style={{
          display: "flex",
          overflowX: "auto",
          overflowY: "hidden",
      ...
      >
        {/* Kategori select'i daima ilk sırada ve min genişlikte */}
        <select
          className="filter-select"
          value={category}
          onChange={e => setCategory(e.target.value)}
          style={{ minWidth: 110, flex: "0 0 110px" }}
        >
              ...
              {services.map((service) => (
                ...
                >
                  <div className="flex flex-col gap-1 px-3 py-4 flex-1 relative">
                    ...
                    <div className="flex items-center justify-between mt-auto pt-2" style={{minHeight: 28}}>
                      ...
                      <span className="font-extrabold text-teal-600 text-base text-right" style={{fontSize: 15, minWidth: 48}}>
                        {service.price} ₺
                      </span>
                    </div>
                  </div>
